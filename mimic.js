...
function onStop() {
  ...
  // TODO(optional): You can stop the game as well
  clear_emoji_timer();
};
...
detector.addEventListener("onImageResultsSuccess", function(faces, image, timestamp) {
  ...
  if (faces.length > 0) {
   ...
   drawEmoji(canvas, image, faces[0]);
    ...
    Check_Emoji(faces[0].emojis.dominantEmoji);
  }
});
...
function drawEmoji(canvas, img, face) {
  ...
  var x1 = face.featurePoints[16].x; // outer right eye
  var y1 = face.featurePoints[16].y; // outer right eye
  var x2 = face.featurePoints[19].x; // outer left eye
  var eyes_distance = Math.abs(x1 - x2);

  if(eyes_distance < ctx.canvas.width/5)
        ctx.font="30px Verdana";
  else
  ...
  ctx.fillText(face.emojis.dominantEmoji,x1 - 50, y1 - 50);
}
...
function clear_emoji_timer()
{
    clearInterval(emojiTimer);
}

function Check_Emoji(emoji)
{
    if(toUnicode(emoji) == current_emoji)
    {
        clear_emoji_timer();
        emoji_timeout = 0;
        ...
        Change_Emoji();
        run_emoji_timer();
        var audio = new Audio('yes-1.wav');
        audio.play();
    }
    else if(emoji_timeout == 1)
    {
        Change_Emoji();
        emoji_timeout = 0;
    }
...
{
    var code = Math.floor((Math.random() * 13) );
    console.log(code);
    setTargetEmoji(emojis[code]);
    current_emoji = emojis[code];
    count_games = count_games + 1;
    setScore(count_success, count_games);
}

setTimeout(function(){ warmup_timeout = 1; }, 10000);